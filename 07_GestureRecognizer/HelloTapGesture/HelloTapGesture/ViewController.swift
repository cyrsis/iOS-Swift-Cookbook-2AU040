...
class ViewController: UIViewController {
    ...
    @IBAction func tapAction(sender: UITapGestureRecognizer) {
        let touchPoint = sender.locationOfTouch(0, inView: sender.view)
    ...
    myTapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(ViewController.programTap(_:)))
    //設定需要幾根手指觸發點擊事件
