...
class ViewController: UIViewController,UIScrollViewDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        //設定三個畫面的顏色
        let viewColors = [UIColor.redColor(),UIColor.greenColor(),UIColor.blueColor()]
        //在ScrollView上加入三個畫面
        for i in 0..<3{
            let oneView = UIView(
                frame: CGRect(
        ...
        //設定PageControl顏色
        codePageControl.pageIndicatorTintColor = UIColor.blackColor()
        //設定目前頁面的顏色
        codePageControl.currentPageIndicatorTintColor = UIColor.lightGrayColor()
        //設定總頁數
        codePageControl.numberOfPages = 3
        //設定目前的頁數
        codePageControl.currentPage = 0
        //設定改動頁數後要執行的方法
        codePageControl.addTarget(self, action: #selector(ViewController.codePageControlAction(_:)), forControlEvents: .ValueChanged)
        //把PageControl加到畫面上
