...
class ViewController: UIViewController {
    ...
    @IBAction func myStepperAction(sender: UIStepper) {
    ...
    override func viewDidLoad() {
        ...
        //設定是否持續偵測計數事件的發生
        codeStepper.continuous = true
        //設定是否到達最大值後跳回最小值繼續計數
        codeStepper.wraps = true
        //設定按下計數器會執行的方法
        codeStepper.addTarget(self, action: #selector(ViewController.codeStepperAction(_:)), forControlEvents: .ValueChanged)
        //把計數器加到畫面上
