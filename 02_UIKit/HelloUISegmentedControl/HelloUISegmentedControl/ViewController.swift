...
class ViewController: UIViewController {
    ...
    @IBAction func myToggleAction(sender: UISegmentedControl) {
    ...
    override func viewDidLoad() {
        ...
        //加入第一個選項
        codeToggle.insertSegmentWithTitle("One", atIndex: 0, animated: false)
        //加入第二個選項
        codeToggle.insertSegmentWithTitle("Two", atIndex: 1, animated: false)
        //設定預設選項為 0
        codeToggle.selectedSegmentIndex = 0
        //設定互動時會觸發的程式碼
        codeToggle.addTarget(self, action: #selector(ViewController.codeToggleAction(_:)), forControlEvents: .ValueChanged)
